...
import { inventoryRepository } from '../api/LocalStorageInventoryRepository';
import {
  setRootStockOnly,
  setStockForGroupByTarget,
  updateItemWithGroupSync,
} from '../model/itemSync';
import { buildPriceHistoryEntry, normalizePriceItem } from '../../../shared/lib/price';
import { generateId } from '../../../shared/lib/id';
import { logInfo, logWarn } from '../../../shared/lib/logger';
...
export function useInventory() {
  ...
  useEffect(() => {
    setItemsState(inventoryRepository.getItems().map(withPriceMetadata));
    setCategoriesState(inventoryRepository.getCategories());
    setActivitiesState(inventoryRepository.getActivities());
  }, []);

  const saveItems = useCallback((newItems: InventoryItem[]) => {
    setItemsState(newItems);
    inventoryRepository.saveItems(newItems);
  }, []);
  ...
  const addItem = useCallback((item: Omit<InventoryItem, 'id' | 'createdAt'>) => {
    ...
    inventoryRepository.addActivity({
      itemId: newItem.id,
      itemName: newItem.name,
      type: 'added',
      details: `${newItem.stock}${(newItem as any).unit || '個'} 追加`
    });
    refreshActivities();
  ...
  const clearActivities = useCallback(() => {
    inventoryRepository.clearActivities();
    refreshActivities();
  }, [refreshActivities]);

  const deleteActivity = useCallback((id: string) => {
    inventoryRepository.deleteActivity(id);
    refreshActivities();
  }, [refreshActivities]);

  const updateActivity = useCallback((id: string, updates: Partial<ActivityEntry>) => {
    inventoryRepository.updateActivity(id, updates);
    refreshActivities();
  }, [refreshActivities]);
