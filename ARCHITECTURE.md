...
これらは、ファイル名が似ていても「どのフォルダに入っているか」がそのままそのファイルの「役割と権限」を表すようになっています。

---

## 5. このドキュメントの運用ルール

アーキテクチャの変更や、新しいディレクトリパターンの導入が必要になった場合は、以下の手順で本ドキュメントを更新してください。

1.  **既存ルールの再検証**: 新しい変更が FSD の「単方向依存」ルールを破壊していないか確認する。
2.  **具体例の追記**: ルールを抽象的に書くだけでなく、本ドキュメントの「詳細なディレクトリ構成」や「ファイル・ディレクトリ命名に関する方針」のセクションに、具体的なパスを追記する。
3.  **GEMINI.md/AGENTS.md への波及確認**: 本ドキュメントの変更内容が AI への指示（[GEMINI.md](./GEMINI.md) や [AGENTS.md](./AGENTS.md)）と矛盾していないか確認し、必要に応じてそれらも更新する。
4.  **不変の真理**: 構造の「美しさ」よりも、将来の開発者（または AI）が「どこに何があるか一瞬でわかること」を最優先する。
